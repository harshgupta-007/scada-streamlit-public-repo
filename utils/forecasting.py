...
def build_intraday_forecast(
    ...
    peak_threshold = recent_daily["peak_demand"].mean() + recent_daily["peak_demand"].std(ddof=0)
    ramp_threshold = recent_ramp.mean() + recent_ramp.std(ddof=0) if not recent_ramp.empty else forecast_max_ramp + 1
    demand_bias = profile["weather_adjustment"].abs().mean() if "weather_adjustment" in profile.columns else 0.0
